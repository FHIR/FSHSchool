...
| instanceOptions | N/A | A `key: value` pair, where key is either `setMetaProfile` (options are `always`, `never`, `inline-only`, `standalone-only`) or `setId` (options are `always` or `standalone-only`). 
