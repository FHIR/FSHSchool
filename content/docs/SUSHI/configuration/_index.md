...
In addition to standard version identifiers, the following three special versions are supported:
  ...
  * `current$branchname`: indicates that the dependency should be loaded from the last successful auto-build of the `branchname` branch (e.g., for the branch named `my-test-branch`, use `current$my-test-branch`).
...
| dependencies | dependsOn | A `key: value` pair, where key is the package id and value is the version (or `dev`/`current`/`current$branchname`). For advanced use cases, the value can be an object with keys for `id`, `uri`, `version`, and `extension`. For R5 IG resources, the key `reason` can also be provided. |
